...
namespace LightsOut {
	...
	class LightsOut : MonoBehaviour {
      ...
      if (inputFieldIsFocused) return;

      if (Input.GetKeyDown(KeyCode.P)) { // changed from L to P, to avoid conflicts with RCS Build Aid (translation keys) and RPM Camera (toggle all camera FOV's)
				SetTime((currentTime == EditorTime.Day) ? EditorTime.Night : EditorTime.Day);
			}
